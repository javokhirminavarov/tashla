...
export default function HabitCard({
  ...
  return (
    <button
      onClick={onSelect}
      className={`flex-1 min-w-[100px] bg-bg-surface rounded-2xl p-4 flex flex-col items-center gap-1.5 border shadow-card transition-all active:scale-[0.97] transition-transform duration-100 ${
        selected
          ? "border-brand/50 ring-1 ring-brand/30"
          : "border-white/[0.06]"
      }`}
    >
      <span className="text-sm font-medium text-text-secondary">{t(`habits.${ht}`)}</span>
